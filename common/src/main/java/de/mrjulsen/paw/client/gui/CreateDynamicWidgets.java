...
public class CreateDynamicWidgets {
    ...
    public static void renderContainer(Graphics graphics, int x, int y, int w, int h, ContainerColor color) {
        ...
        renderNineSliced(graphics, x, y, w, h, 0, 7, CMRGui.GUI_WIDTH, CMRGui.GUI_HEIGHT, 2, CMRGui.GUI, false);
    }

    public static void renderTitleBar(Graphics graphics, int x, int y, int w, int h, BarColor color) {
        renderNineSliced(graphics, x, y, w, h, color.u, color.v, CMRGui.GUI_WIDTH, CMRGui.GUI_HEIGHT, 3, CMRGui.GUI, true);
    }
    ...
    public static void renderTextHighlighted(Graphics graphics, int x, int y, Font font, Component text, int color) {
        ...
        GuiUtils.drawString(graphics, font, x + 3, y + 3, text, ColorUtils.brightnessDependingFontColor(color, 0xFFFFFFFF, 0xFF000000), EAlignment.LEFT, false);
    }

    public static void renderIconSlot(Graphics graphics, int x, int y, int w, int h) {
        renderNineSliced(graphics, x, y, w, h, 21, 0, CMRGui.GUI_WIDTH, CMRGui.GUI_HEIGHT, 1, CMRGui.GUI, true);
    }

    public static void renderTextBoxArrow(Graphics graphics, int x, int y) {
        GuiUtils.drawTexture(CMRGui.GUI, graphics, x - 3, y + 5, 4, 8, 60, 54, 4, 8, CMRGui.GUI_WIDTH, CMRGui.GUI_HEIGHT);
    }

    public static void renderVerticalSeparator(Graphics graphics, int x, int y, int h, BarColor color) {
        GuiUtils.drawTexture(CMRGui.GUI, graphics, x, y, 1, h, color.u + 5, color.v + 3, 1, 1, CMRGui.GUI_WIDTH, CMRGui.GUI_HEIGHT);
        GuiUtils.drawTexture(CMRGui.GUI, graphics, x + 1, y, 1, h, color.u + 1, color.v + 3, 1, 1, CMRGui.GUI_WIDTH, CMRGui.GUI_HEIGHT);
    }
